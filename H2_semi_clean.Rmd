...
`r sprintf("%.2f",t1)`,
*p* `r paste(p_report(p2))`.
Again, to interpret this, we conducted a series of correlations to test if age was related to symbolization across all countries.  There was a significant correlation between age and symbolization in `r total_sig` countries. Of these, there was a positive correlation between age and symbolization in `r pos` countries, and a negative correlation in `r neg` countries (see Supplementary Materials [Table S_X](https://cillianmiltown.github.io/moral_ID_gender/drafting_supplementary_analysis.html#Age__Symbolization)).
